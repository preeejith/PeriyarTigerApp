...
class _UploadDataSync extends State<UploadDataSync> {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
          ...
          body: Column(
            ...
            children: [
              BlocConsumer<BlocSyncOffline, StateSyncOffline>(
                  buildWhen: (((previous, current) =>
                      current is DataFoundState ||
                      current is UploadingData ||
                      current is UploadingFinishedState ||
                      current is UploadingNotFinishedState)),
                  ...
                  listenWhen: ((previous, current) =>
                      current is UploadingNotFinishedState ||
                      current is UploadingFinishedState ||
                      current is CheckingSync ||
                      current is DataNotFoundState ||
                      current is NoInternetState),
                  listener: (context, state) async {
                    ...
                    if (state is NoInternetState) {
                      _showAlert(
                          context,
                          'No internet',
                          'connection failed, please check the internet',
                          false);
                      BlocProvider.of<BlocSyncOffline>(context)
                          .add(UploadEventInitial());
                    }
                  }),
              // BlocBuilder<BlocSyncOffline, StateSyncOffline>(
              //     buildWhen: ((previous, current) =>
              //         current is DataFoundState ||
              //         current is BusDataUploading ||
              //         current is BusDataUploadedState),
              //     builder: ((context, state) {
              //       return context.read<BlocSyncOffline>().busLength != 0
              //           ? Padding(
              //               padding: const EdgeInsets.all(12.0),
              //               child: AppCard(
              //                 outLineColor: HexColor('#292929'),
              //                 child: state is BusDataUploading
              //                     ? Row(
              //                         mainAxisAlignment:
              //                             MainAxisAlignment.spaceBetween,
              //                         children: [
              //                           new Text(
              //                             'Uploading Bus Trip Data',
              //                             style: TextStyle(fontSize: 18.0),
              //                           ),
              //                           SizedBox(
              //                             width: 36,
  ...
  void getData(BuildContext context) async {
    ...
    if (allDataList.length != 0) {
      BlocProvider.of<BlocSyncOffline>(context).add(DataFound(
          ...
          failedLength: failedData.length,
          busUploadLength: busUpload.length));
    } else {
  ...
  checkDb() async {
    ...
    if (finalList.length == 0) {
      ...
      print("++++++++++++++++++++++++++++++++++++++++");
      print(finalList);
      print(finalList[0]['totalAmount']);
      print(finalList[0]['reason']);
      print(finalList[0]['ticketId']);
    ...
    if (list.length == guestData.length) {
      ...
      for (int k = 0; k < list.length; k++) {
        if (guestData[k]['image'] != 'Empty') {
          ...
          await db!.deleteFromBookings(tableId);
        }
      }
    ...
    for (int i = 0; i < allDataList.length; i++) {
      ...
      if (theStatus == true) {
        try {
          ...
          var url = '/reception/sync/ticket';
          // Map data = {};
          Map data = {
            'bus': allDataList[i]['busId'],
  ...
  uploadBusData() async {
    context.read<BlocSyncOffline>().add(UploadingBusData());
    List busUpload = await db!.getBusFinalData();
    // List busUploadDummy = busUpload;
    if (busUpload.isNotEmpty) {
      try {
        ...
        Fluttertoast.showToast(msg: "Sync completed");
        await db!.clearBusTable();
      } catch (e) {
        print("Bus Data Upload Exception -- $e");
      }

      context.read<BlocSyncOffline>().add(BusDataUploaded());

      // if (busUploadDummy.isEmpty) {
  ...
  //   if (busUpload.isNotEmpty) {
  //     for (int index = 0; index < busUpload.length; index++) {
  //       if (busUpload[index].isActive == 'true') {
  //         var theStatus = await checkConnection();
  //         CommonModel commonModel = CommonModel();
  //         if (theStatus == true) {
  //           try {
  ...
  //               'tripCount': busUpload[index].tripCount,
  //               'tickets': busUpload[index].tickets,
  //               'startTime': busUpload[index].startTime,
  //               'endTime': busUpload[index].endTime,
  //               'noOfPassengers': busUpload[index].noOfPassengers,
  //             };
  //             print(busUploadData);
  //             commonModel = await AuthRepository().sendBookedData(
